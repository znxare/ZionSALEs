...
interface Props {
  ...
  profiles: Profile[];
  onClose: () => void;
  onSaved: (lead: HospitalityLead) => void;
}

export default function EditHospitalityLeadModal({ lead, profiles, onClose, onSaved }: Props) {
  ...
  async function save() {
    ...
    try {
      if (!duplicate && normalizePhone(form.phone) !== normalizePhone(lead.phone)) {
        const existing = await findHospitalityLeadByPhone(form.phone, lead.id);
        if (existing) {
      ...
      const patch: Partial<HospitalityLead> = {
        name: form.name.trim(),
      ...
      const updated = await updateHospitalityLead(lead.id, patch);
      onSaved(updated);
    } catch (e) {
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/30 backdrop-blur-sm sm:items-center" onClick={onClose}>
      ...
      >
        <div className="sticky top-0 flex items-center justify-between border-b border-gray-100 bg-white px-5 py-4">
          <h2 className="font-display text-lg font-bold tracking-tight text-gray-900">Edit Hospitality Lead</h2>
        ...
        <div className="space-y-4 px-5 py-5">
          ...
          <Select label="Lead source" value={form.source} onChange={(v) => setForm({ ...form, source: v as HospitalitySource })} options={HOSPITALITY_SOURCES} />
          <Select label="Status" value={form.status} onChange={(v) => setForm({ ...form, status: v as LeadStatus })} options={HOSPITALITY_STATUSES} />
