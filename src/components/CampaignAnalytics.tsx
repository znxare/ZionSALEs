...
import {
  ...
  Archive, ArchiveRestore, Calendar, Phone,
} from 'lucide-react';
    ...
    <div className="animate-fade-in">
      ...
      {dateFilteredLeads.length > 0 && (
        <div className="mb-6 rounded-2xl border border-black/5 bg-white p-5 card-shadow">
          ...
          <LineChart data={overallTrendData} color="#1f6f43" height={200} />
          ...
          <div key={m.campaign.id} className="rounded-2xl border border-black/5 bg-white p-5 card-shadow transition hover:shadow-md">
            ...
            <div className="mt-3 flex items-center gap-2">
              <span className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${
                ...
                {m.qualityRating === 'Excellent' ? '🟢' : m.qualityRating === 'Good' ? '🟡' : m.qualityRating === 'Average' ? '🟠' : '🔴'} {m.qualityRating}
              </span>
            ...
            <p className="mt-1 text-[10px] text-gray-400">Quality = (Hot + Warm + Booked leads ÷ total leads) × 100. 60%+ = Excellent, 40%+ = Good, 20%+ = Average, below = Poor.</p>
      ...
      {comparisonData.length > 0 && (
        <div className="mb-6 rounded-2xl border border-black/5 bg-white p-5 card-shadow">
          ...
          <BarChart data={comparisonData} color="#1f6f43" />
        </div>
      )}
