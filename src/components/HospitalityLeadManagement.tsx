...
import {
  ...
  CheckSquare, Square, BarChart3, Download, Plus,
} from 'lucide-react';
import type { LeadStatus, Profile } from '@/lib/supabase';
import {
  HOSPITALITY_SOURCES, HOSPITALITY_STATUSES, isHospitalityFollowUpRequired,
  updateHospitalityLead, deleteHospitalityLead, scheduleHospitalityFollowUp,
  fetchHospitalityActivities, type HospitalityLead, type HospitalitySource,
...
type CategoryTab = 'all' | 'hot' | 'warm' | 'cold' | 'calling' | 'booked' | 'junk' | 'dead';
...
function matchesTab(l: HospitalityLead, tab: CategoryTab): boolean {
  switch (tab) {
    ...
    case 'booked': return !!l.booked_at;
...
export default function HospitalityLeadManagement({ leads, profiles, onOpenLead, onChanged }: Props) {
  ...
  const [filters, setFilters] = useState<FilterState>({ sources: [], statuses: [], bookedOnly: false });
  const [selected, setSelected] = useState<Set<string>>(new Set());
  const [editing, setEditing] = useState<HospitalityLead | null>(null);
  const [adding, setAdding] = useState(false);
  const [followUpFor, setFollowUpFor] = useState<HospitalityLead | null>(null);
  ...
  const tabCounts = useMemo(() => {
    const counts: Record<CategoryTab, number> = { all: 0, hot: 0, warm: 0, cold: 0, calling: 0, booked: 0, junk: 0, dead: 0 };
    leads.forEach((l) => {
      tabConfig.forEach((t) => {
  ...
  const filtered = useMemo(() => {
    ...
    let result = leads.filter((l) => {
      ...
      if (filters.sources.length && !filters.sources.includes(l.source as HospitalitySource)) return false;
  ...
  function clearFilters() {
    setFilters({ sources: [], statuses: [], bookedOnly: false });
    setSearch('');
    setPage(0);
  }

  const activeFilterCount = filters.sources.length + filters.statuses.length + (filters.bookedOnly ? 1 : 0);

  async function applyBulkStatus() {
    if (!bulkStatus || selected.size === 0) return;
    setActionError(null);
    try {
      await Promise.all([...selected].map((id) => updateHospitalityLead(id, { status: bulkStatus as LeadStatus })));
      setBulkStatus('');
      setSelected(new Set());
      onChanged();
    } catch (e) {
  ...
  async function applyBulkFollowUp(when: string, summary: string) {
    ...
    setActionError(null);
    try {
      await Promise.all([...selected].map((id) => scheduleHospitalityFollowUp({ id, next_followup_at: when } as HospitalityLead, when, summary)));
      setFollowUpFor(null);
      setSelected(new Set());
      onChanged();
    } catch (e) {
  ...
  async function bulkDelete() {
    ...
    setActionError(null);
    try {
      await Promise.all([...selected].map((id) => deleteHospitalityLead(id)));
      setSelected(new Set());
      onChanged();
    } catch (e) {
  ...
  function exportCsv() {
    ...
    a.href = url; a.download = 'hospitality-leads-export.csv'; a.click();
  ...
  async function exportFullCsv() {
    ...
    try {
      ...
      const cols = ['Name', 'Phone', 'Email', 'City', 'Source', 'Status', 'Budget', 'Next Follow-up', 'Last Contacted', 'Last Activity', 'Booked', 'Notes', 'Created'];
      const lines = [cols.join(',')];

      const CHUNK_SIZE = 20;
      const activitiesByLead = new Map<string, Awaited<ReturnType<typeof fetchHospitalityActivities>>>();
      for (let i = 0; i < rows.length; i += CHUNK_SIZE) {
        const chunk = rows.slice(i, i + CHUNK_SIZE);
        const results = await Promise.all(chunk.map((l) => fetchHospitalityActivities(l.id).catch(() => [])));
      ...
      for (const l of rows) {
        ...
        lines.push([
          l.name, l.phone, l.email ?? '', l.city ?? '', l.source, l.status, l.budget ?? '',
      ...
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url; a.download = `hospitality-leads-full-export-${new Date().toISOString().slice(0, 10)}.csv`; a.click();
      URL.revokeObjectURL(url);
    } finally {
      setExporting(false);
    }
  ...
  async function inlineStatus(lead: HospitalityLead, status: LeadStatus) {
    setActionError(null);
    try {
      const patch: Partial<HospitalityLead> = { status };
      if (status === 'Dead' || status === 'Junk') patch.next_followup_at = null;
      await updateHospitalityLead(lead.id, patch);
      onChanged();
    } catch (e) {
  ...
  async function inlineFollowUp(lead: HospitalityLead, value: string) {
    if (!value) return;
    setActionError(null);
    try {
      await updateHospitalityLead(lead.id, { next_followup_at: new Date(value).toISOString() });
      onChanged();
    } catch (e) {
  ...
  return (
    <div className="animate-fade-in space-y-5">
      ...
      <div className="rounded-2xl border border-black/5 bg-white p-5 card-shadow">
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            ...
            className="hidden shrink-0 items-center gap-1.5 rounded-full bg-rose-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-rose-700 sm:flex"
          >
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {(['all', 'today', 'yesterday', 'this_week', 'this_month', 'last_month', 'custom'] as const).map((p) => (
            <button key={p} onClick={() => { setRangePreset(p); setPage(0); }} className={`rounded-full px-3 py-1.5 text-[12px] font-medium transition ${rangePreset === p ? 'bg-rose-600 text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
              {p === 'all' ? 'All Time' : p.replace('_', ' ').replace(/\b\w/g, (c) => c.toUpperCase())}
            </button>
          ))}
          {rangePreset === 'custom' && (
            <div className="flex items-center gap-2">
              <input type="date" value={customStart} onChange={(e) => { setCustomStart(e.target.value); setPage(0); }} className="rounded-lg border border-gray-200 px-2.5 py-1.5 text-[13px] outline-none focus:border-rose-300" />
              <span className="text-gray-400">→</span>
              <input type="date" value={customEnd} onChange={(e) => { setCustomEnd(e.target.value); setPage(0); }} className="rounded-lg border border-gray-200 px-2.5 py-1.5 text-[13px] outline-none focus:border-rose-300" />
            </div>
          )}
          <button
            onClick={() => setShowAnalytics((v) => !v)}
            className={`flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-[12px] font-medium transition ${showAnalytics ? 'border-rose-200 bg-rose-50 text-rose-700' : 'border-gray-200 bg-gray-50 text-gray-600 hover:bg-gray-100'}`}
          >
          ...
          <button
            onClick={() => setAdding(true)}
            className="ml-auto flex items-center gap-1.5 rounded-full bg-rose-600 px-3.5 py-2 text-[12px] font-semibold text-white shadow-sm transition hover:bg-rose-700 sm:hidden"
          >
        ...
        {tabConfig.map((t) => {
          ...
          return (
            <button
              ...
              className={`flex shrink-0 items-center gap-1.5 rounded-full px-3 py-2 text-[13px] font-semibold transition ${active ? 'bg-rose-600 text-white shadow-sm' : 'bg-white text-gray-500 ring-1 ring-black/5 hover:text-gray-700'}`}
            >
              {t.label}
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-full border border-black/5 bg-white py-2.5 pl-11 pr-4 text-sm font-medium text-gray-900 outline-none card-shadow placeholder:text-gray-400 focus:border-rose-200"
          />
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => setShowFilters((v) => !v)}
            className={`relative flex items-center gap-1.5 rounded-full border px-3.5 py-2.5 text-sm font-medium transition ${activeFilterCount > 0 ? 'border-rose-200 bg-rose-50 text-rose-700' : 'border-black/5 bg-white text-gray-600 card-shadow'}`}
          >
            <Filter className="h-4 w-4" /> Filters
            {activeFilterCount > 0 && <span className="grid h-5 w-5 place-items-center rounded-full bg-rose-600 text-[11px] text-white">{activeFilterCount}</span>}
          </button>
          <div className="relative">
            <select value={sort} onChange={(e) => setSort(e.target.value as SortKey)} className="appearance-none rounded-full border border-black/5 bg-white py-2.5 pl-3.5 pr-9 text-sm font-medium text-gray-600 outline-none card-shadow focus:border-rose-200">
          ...
          <div className="flex overflow-hidden rounded-full border border-black/5 bg-white card-shadow">
            <button onClick={() => setView('table')} className={`px-2.5 py-2.5 transition ${view === 'table' ? 'bg-rose-600 text-white' : 'text-gray-400 hover:text-gray-600'}`}><TableIcon className="h-4 w-4" /></button>
            <button onClick={() => setView('card')} className={`px-2.5 py-2.5 transition ${view === 'card' ? 'bg-rose-600 text-white' : 'text-gray-400 hover:text-gray-600'}`}><LayoutGrid className="h-4 w-4" /></button>
          </div>
        </div>
      </div>
      ...
      {showFilters && (
        <div className="mb-4 animate-fade-up rounded-2xl border border-black/5 bg-white p-4 card-shadow">
          <div className="mb-3 flex items-center justify-between">
            ...
            {activeFilterCount > 0 && <button onClick={clearFilters} className="text-[12px] font-semibold text-rose-600 hover:text-rose-700">Clear all</button>}
          </div>
          <div className="space-y-3">
            <FilterGroup label="Lead Source">
              {HOSPITALITY_SOURCES.map((s) => <Chip key={s} active={filters.sources.includes(s)} onClick={() => toggleFilter('sources', s, filters.sources)} label={s} />)}
            </FilterGroup>
            <FilterGroup label="Status">
              {HOSPITALITY_STATUSES.map((s) => <Chip key={s} active={filters.statuses.includes(s)} onClick={() => toggleFilter('statuses', s, filters.statuses)} label={s} />)}
            </FilterGroup>
      ...
      {/* Bulk action bar */}
      {selected.size > 0 && (
        <div className="mb-4 flex flex-wrap items-center gap-2 animate-fade-up rounded-2xl border border-rose-200 bg-rose-50/70 px-4 py-3">
          ...
          {selected.size === paged.length && filtered.length > paged.length && (
            <button onClick={() => setSelected(new Set(filtered.map((l) => l.id)))} className="text-[12px] font-semibold text-rose-700 underline decoration-rose-300 underline-offset-2 hover:text-rose-800">
              Select all {filtered.length} matching filters
            </button>
          )}
          <div className="mx-1 h-5 w-px bg-rose-200" />
          <div className="relative">
            <select value={bulkStatus} onChange={(e) => setBulkStatus(e.target.value as LeadStatus)} className="appearance-none rounded-lg border border-rose-200 bg-white py-1.5 pl-3 pr-8 text-[13px] font-medium text-gray-700 outline-none">
              <option value="">Change status…</option>
              {HOSPITALITY_STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}
            </select>
          </div>
          {bulkStatus && <button onClick={applyBulkStatus} className="rounded-lg bg-rose-600 px-3 py-1.5 text-[13px] font-semibold text-white">Apply</button>}
              ...
              <thead className="sticky top-0 z-10 bg-gray-50/90 backdrop-blur">
                <tr className="text-left text-[11px] font-semibold uppercase tracking-wide text-gray-400">
                  <th className="w-10 px-4 py-3">
                    <button onClick={toggleAll}>{selected.size === paged.length && paged.length > 0 ? <CheckSquare className="h-4 w-4 text-rose-600" /> : <Square className="h-4 w-4 text-gray-300" />}</button>
                  </th>
                    ...
                    <tr key={l.id} className="group border-t border-gray-100 transition hover:bg-gray-50/60">
                      <td className="px-4 py-3">
                        <button onClick={() => toggleSelected(l.id)}>
                          {selected.has(l.id) ? <CheckSquare className="h-4 w-4 text-rose-600" /> : <Square className="h-4 w-4 text-gray-300" />}
                        </button>
                      </td>
                      <td className="px-4 py-3">
                        <button onClick={() => onOpenLead(l.id)} className="text-left font-semibold text-gray-900 hover:text-rose-600">{l.name}</button>
                      ...
                      <td className="px-4 py-3">
                        {isHospitalityFollowUpRequired(l) ? (
                          <input
                            type="datetime-local"
                            defaultValue={toLocalInputValue(l.next_followup_at)}
                            onChange={(e) => inlineFollowUp(l, e.target.value)}
                            className={`w-[150px] rounded-lg border-0 bg-transparent px-1 py-1 text-[12px] font-medium outline-none hover:bg-gray-50 focus:bg-gray-50 focus:ring-1 focus:ring-rose-200 ${overdue ? 'text-red-600' : 'text-gray-700'}`}
                          />
                        ) : (
              ...
              <div key={l.id} className="group rounded-2xl border border-black/5 bg-white p-4 card-shadow transition hover:shadow-md">
                <div className="flex items-start justify-between gap-2">
                  <button onClick={() => onOpenLead(l.id)} className="flex items-center gap-2.5 text-left">
                    <div className="grid h-10 w-10 place-items-center rounded-full bg-rose-50 text-sm font-bold text-rose-700">{l.name.charAt(0).toUpperCase()}</div>
                    <div>
      ...
      {totalPages > 1 && (
        <div className="mt-5 flex items-center justify-between text-sm">
          ...
          <div className="flex gap-1">
            ...
            <span className="rounded-lg bg-rose-50 px-3 py-1.5 font-semibold text-rose-700">{page + 1} / {totalPages}</span>
      ...
      {/* Modals */}
      {adding && (
        <AddHospitalityLeadModal profiles={profiles} onClose={() => setAdding(false)} onCreated={() => { setAdding(false); onChanged(); }} />
      )}
      {editing && (
        <EditHospitalityLeadModal lead={editing} profiles={profiles} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); onChanged(); }} />
      )}
      {followUpFor && (
        <FollowUpSheet
          ...
          onPick={async (when, summary) => {
            ...
            else { await scheduleHospitalityFollowUp(followUpFor, when, summary); onChanged(); setFollowUpFor(null); }
          }}
        />
      )}
    </div>
...
function InlineStatusDropdown({ lead, onChange }: { lead: HospitalityLead; onChange: (s: LeadStatus) => void }) {
...
function Chip({ active, onClick, label }: { active: boolean; onClick: () => void; label: string }) {
  return (
    <button onClick={onClick} className={`rounded-full px-3 py-1.5 text-[12px] font-medium transition ${active ? 'bg-rose-600 text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{label}</button>
  );
}
